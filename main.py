...
df_bike_acc_all['geometry'] = df_bike_acc_all['geometry'].buffer(10, resolution=2)
...
# - buffered RVA
df_bike_acc_all.to_file(path + dir_output + gpkg_name_buf, layer='buf_acc', driver='GPKG')
...
df_rva.to_file(path + dir_output + gpkg_name_buf, layer='buf_rva', driver='GPKG')
...
# Spatial join (1): bike accidents for all roads without speed limit -> drop duplicates by considering road rank (highest ranked road 'wins')
df_acc_x_road_wo_speed = gpd.sjoin(df_bike_acc_all, df_roads_wo_speed, how='inner', predicate='intersects', rsuffix='road')
df_acc_x_road_wo_speed = df_acc_x_road_wo_speed.sort_values(['rank'], ascending=False).drop_duplicates(subset=['GUID'], keep='first')
...
df_acc_x_rva.to_file(path + dir_output + gpkg_name_buf, layer='buf_acc_rva', driver='GPKG')
print("- GeoPackage updated")
